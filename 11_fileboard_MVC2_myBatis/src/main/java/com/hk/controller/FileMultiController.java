...
//                multi.getParameter("title"); // text들도 multi객체로 받아야함
             ...
             while(files.hasMoreElements()) { // 객체 안에 값이 있는지 확인해서 실행하는 방식
            	 String fileName = (String)files.nextElement(); // 각각의 파일 name 값을 구한다.
            	 System.out.println("filename 속성값 : " + fileName);
            	 if(multi.getFile(fileName)!=null) { // 실제 첨부된 파일만 처리
            	 
		             // DB에 파일정보 추가하기
		             // 1. 원본파일명 구하기
		             // "filename"은 페이지에서 전달될 때 정의했던 이름 name="filename"
		             ...
		             // 4. DB에 정보 추가하기
		             isS=dao.insertFile(new FileDto(0, origin_name, stored_name, file_size, null));
