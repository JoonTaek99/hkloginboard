...
// jquery ajax() 사용하여 구현
$(function() { // onload 이벤트 기능 --> window.onload=function(){}
	// 상세 내용 조회 : ajax로 구현
	...
	$("td>a").hover(function(){
		...
		$.ajax({
			...
			async:false, // ajax 메서드를 동기식으로 실행
			success:function(obj){ // 통신 성공하면 실행
				 // {"dto":{seq:5,id:"hk",title:"제목"...}}
...
<%-- 				<a href="boardList.do?pnum=${pMap.nextPageNum}">▶</a> --%>
	...
	<form action="">
		<div id="ajaxForm">
			...
			<label>제  목:</label><input class="form-control" type="text" id="title"/>
			<label>내  용:</label><textarea class="form-control" rows="3" cols="60" id="content"></textarea>
			<label></label>
		</div>
	</form>
</div>
